...
def default_query_resolver(type, page=None, perPage=10, sortField=None, sortOrder=None, filter=None, **kwargs):
    qs = type.objects.all()
    ...
    if filter is not None:
        f_items = {k: v for k, v in filter.items(
...
class BaseQuery(ObjectType):
    ...
    class ListMetaField(Field):
        def __init__(self, filter, name, resolver):
            super().__init__(ListMetadata, page=Int(), perPage=Int(),
                             sortField=String(), sortOrder=String(), filter=filter(), name=name, resolver=resolver)

    class QueryList(List):
        def __init__(self, type, filter, resolver):
            super().__init__(type, page=Int(), perPage=Int(),
                             sortField=String(), sortOrder=String(), filter=filter(), resolver=resolver)
