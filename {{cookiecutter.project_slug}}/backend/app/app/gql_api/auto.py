...
def schema_operations_builder(operationName, operationModule, operationBase,
                              clsName):

    op_base_classes = build_base_classes(operationName, operationModule,
                                         operationBase, clsName)

    if len(op_base_classes) <= 1:
        raise ValueError(
            "Found no '{0}' classes in '{1}' module of subdirectories.".format(
...
def build_base_classes(operationName, operationModule, operationBase, clsName):
    ...
    subdirectories = [
        ...
        and x != '__pycache__' and x != 'root'
    ]
    op_base_classes = [OperationAbstract]

    for directory in subdirectories:
        try:
            package_path = 'app.{0}.{1}.{2}'.format("gql_api", directory,
                                                    operationModule)
            module = importlib.import_module(package_path, 'app')
            if module:
                classes = [x for x in getmembers(module, isclass)]
                opers = [
                    x[1] for x in classes
                    if clsName in x[0] and x[0] != operationBase
                ]
